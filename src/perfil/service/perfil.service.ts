...
export class PerfilService {
  ...
  async create(body: CreatePerfilDto) {
    try {
      ...
      const perfilReturn= await this.perfilRepository.findOneBy({id: perfil.id});
      ...
      throw new HttpException({ message: "Não foi possível cadastrar Perfil. ", code: error?.code, erro: error }, HttpStatus.BAD_REQUEST)
    }
  }
  ...
  async update(idPublic: string, body: UpdatePerfilDto) {
    ...
    try {
      // Busca no banco um perfil com o idPublic informado
      const perfilOriginal = await this.perfilRepository.findOneBy({
        idPublic: idPublic
      })
      ...
      // Busca no banco um perfil com o idPublic informado
      const perfil = await queryRunner.manager.findOneBy(Perfil, { idPublic: idPublic })
      ...
      throw new HttpException({ message: 'Não foi possível modificar a Perfil. ', code: error?.code, erro: error }, HttpStatus.BAD_REQUEST)
    } finally {
  ...
  async remove(idPublic: string) {
    try {
      ...
      // Retorna mensagem de erro
      throw new HttpException({ message: 'Não foi possível deletar o Perfil. ', code: error?.code, erro: error }, HttpStatus.BAD_REQUEST)
    }
  }
}
